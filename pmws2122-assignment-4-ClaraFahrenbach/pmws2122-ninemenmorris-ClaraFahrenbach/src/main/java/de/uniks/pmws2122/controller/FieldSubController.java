...
public class FieldSubController {
    ...
    public void init() {
        ...
        this.model.listeners().addPropertyChangeListener(Field.PROPERTY_MEN, this.manPlacedListener);
    }

    public void stop() {
        // Remove property change listeners from field
        this.model.listeners().removePropertyChangeListener(Field.PROPERTY_MEN, this.manPlacedListener);
    }
    ...
    private void gamePhasePlacingManNull() {
        ...
        if (this.modelService.getGame().getCurrentPlayer().getAction().equals(PLAYER_ACTION_PLACING)) {
            ...
            // Check for a mill
            this.modelService.checkMill(this.model.getMen());
    ...
    private void gamePhasePlacingManNotNull() {
        ...
        if (this.modelService.getGame().getCurrentPlayer().getAction().equals(PLAYER_ACTION_REMOVE)) {
            ...
            if (this.model.getMen().getOwner().equals(this.modelService.getGame().getCurrentPlayer().getNext())) {
                // Remove the man
                this.modelService.removeMan(this.model.getMen());
    ...
    private void gamePhaseMovingManNotNull() {
        ...
        if (selectedMan == null && selectedField == null && this.model.getMen().getOwner().equals(currentPlayer) && (playerAction.equals(PLAYER_ACTION_MOVING) || playerAction.equals(PLAYER_ACTION_FLYING))) {
            // Set the current selected man in the ModelService to "this field man"
            this.modelService.setCurrentSelectedMan(this.model.getMen());
        ...
        // and that the action of the current player is "moving" or "flying"
        } else if (selectedMan != null && selectedField == null && this.model.getMen().getOwner().equals(currentPlayer) && this.model.getMen().equals(selectedMan) && (playerAction.equals(PLAYER_ACTION_MOVING) || playerAction.equals(PLAYER_ACTION_FLYING))) {
            ...
            this.view.setFill(this.model.getMen().getColor().equals(COLOR_BLACK) ? Color.BLACK : Color.WHITE);
        // Check that the player action of the current player is "remove" 
            ...
            if (this.model.getMen().getOwner().equals(currentPlayer.getNext())) {
    ...
    private void onFieldMouseHoverEnter(MouseEvent event) {
        // Change the view
        if (this.model.getMen() == null) {
            this.view.setFill(Color.YELLOWGREEN);
