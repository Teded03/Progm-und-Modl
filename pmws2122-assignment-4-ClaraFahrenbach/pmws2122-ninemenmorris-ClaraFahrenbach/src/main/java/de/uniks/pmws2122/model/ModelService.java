...
import de.uniks.pmws2122.ai.Brain;
...
public class ModelService {
    ...
    public void buildGame(String playerNameOne, String playerNameTwo, String gamemode) {

        if (gamemode == PLAYER_VS_PLAYER) {
            this.game = new Game().setName("Epic battle");
        }
        if (gamemode == PLAYER_VS_AI) {
            this.game = new Game().setName("Epic battle vs AI");
        }
        // Create player one
        ...
        // Create player two
        Player playerTwo = new Player() // zweitem Spieler einen Namen geben damit darauf zugegriffen werden kann
        ...
        // if (gamemode == PLAYER_VS_AI) {
        // playerTwo.setName(NAME_AI); //zweitem Spieler Name Computer zuweisen
        // //geschieht schon beim creategame vs ai button
        // }
        ...
        // start the brain
        if (gamemode == PLAYER_VS_AI) {
            brain = new Brain(playerTwo, this);
        }
    }
    ...
    public void startGame(Player beginner) {
        ...
        if (gamemode == PLAYER_VS_AI) {
            if (this.game.getCurrentPlayer().getName() == NAME_AI) {
                brain.turn();
            }
        }
    }
    ...
    public void checkWinner() {
        ...
        if (this.game.getPhase().equals(GAME_PHASE_MOVING)) {
            ...
            for (Player p : this.game.getPlayers()) {
                ...
                // Also check if a player can not move anymore, if so, he lost
                if (p.getMen().stream().noneMatch(
                    (m) -> (m.getPosition().getLeft() != null && m.getPosition().getLeft().getMen() == null) ||
                           (m.getPosition().getRight() != null && m.getPosition().getRight().getMen() == null) ||
                           (m.getPosition().getTop() != null && m.getPosition().getTop().getMen() == null) ||
                           (m.getPosition().getBottom() != null && m.getPosition().getBottom().getMen() == null))) {
                    this.game.setWinner(p.getNext()); //klappt nicht in flying erkennt nicht das es auch aus der eingeklemmten lage rausspringen kann
                }
            }
        }
    }
    ...
    public void nextTurn() {
        ...
        /*
         * if it is a human vs computer game,
         * and it’s the computer’s turn, let the computer make its move
         */
        this.checkNextPhase();
        if (gamemode == PLAYER_VS_AI) {
            if (this.game.getCurrentPlayer().getName() == NAME_AI) {
    ...
    public void checkNextPhase() {
        ...
        // moving, only if the current phase is really "placing"
    ...
    public void placeMan(Field field) {
        ...
        if (field.getMen() != null) {
            return;
        }

        // Check that the game phase / player action is "placing"
        if (!(this.game.getPhase().equals(GAME_PHASE_PLACING)
                && this.game.getCurrentPlayer().getAction().equals(PLAYER_ACTION_PLACING))) {
            return;
        }
    ...
    public void moveMan() {
        // Check if a man and field are selected
        if (this.currentSelectedMan == null || this.currentSelectedField == null) {
            return;
        }

        if (this.currentSelectedField.getMen() != null) {
            return;
        }
        // Check that the game phase is "moving" and player action is "moving" or
        // "flying"
        if (!(this.game.getPhase().equals(GAME_PHASE_MOVING)
                && (this.game.getCurrentPlayer().getAction().equals(PLAYER_ACTION_MOVING)
                        || this.game.getCurrentPlayer().getAction().equals(PLAYER_ACTION_FLYING)))) {
            return;
        }
    ...
    public void removeMan(Man man) {
        // Check that the man is not empty
        if (man == null) {
            return;
        }
        ...
        // Set the player action of the old owner to "flying" if the current game phase
        // is "moving" and the player´s man are 3
        if (this.game.getPhase().equals(GAME_PHASE_MOVING) && oldOwner.getMen().size() == 3) {
            oldOwner.setAction(PLAYER_ACTION_FLYING);
        }

        // Set the player action of the current player depending on the game phase and
    ...
    public void checkVerticalMill(Man lastPutMan) {
        ...
        if (neighbor1.getMen() != null
                && neighbor1.getMen().getColor().equals(currentColor)
                && neighbor2.getMen() != null
                && neighbor2.getMen().getColor().equals(currentColor)) {
